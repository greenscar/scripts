...
# Raghu - Modified the script to search sql directory under rp
# Raghu - Excluding cleanup_all.ksh so that ENV in that script wont be changed
...
do
   ...
   find $ENVT/$a_dir -type f -name "*.ksh" \! -name $ENVT/$a_dir/fw/script/cleanup_all.ksh -exec perl -pi -e "s/\/ENV/\/$1/g" {} \; -print
done

echo "------ Changing ENV to $1 in *.sql --------"
for a_dir in $SQL_LOCS
do
   echo "Changing ENV to $1 in $ENVT/$a_dir/*.sql" 
   find $ENVT/$a_dir -type f -name "*.sql" -exec perl -pi -e "s/\/ENV/\/$1/g" {} \;
done
