...
#
######################
db.define_table('dbs_to_app_instances',
   Field('dbs_id', db.dbs),
   Field('app_instances_id', db.process_instances),
   Field('created_at', 'datetime', default=request.now),
   Field('updated_at', 'datetime', default=request.now),
   migrate=False)
db.dbs_to_app_instances.dbs_id.requires = IS_IN_DB(db, 'dbs.id')
db.dbs_to_app_instances.app_instances_id.requires = IS_IN_DB(db, 'app_instances.id')
db.dbs_to_app_instances.created_at.readable = True
db.dbs_to_app_instances.created_at.writable = False
db.dbs_to_app_instances.updated_at.readable = True
db.dbs_to_app_instances.updated_at.writable = False
######################
#
######################
