...
for($x = 0; $x < @filelist1; $x++)
{
  ...
  #print("1 -> $size1 - $date1 - $time1 - $filename1\n");
  #print("size = $size1\ndate = $date1\ntime = $time1\nfilename = $filename1\n");
  for($y = 0; $y < @filelist2; $y++)
  {
      ...
      #print("2 -> " . @props2[4] . " - " . @props2[5] . " - " . @props2[6] . " - " . @props2[7] . "\n");
      if($time1 == @props2[6] && $filename1 ne @props2[7])
      {
         #print("$x matches $y\n");
         #print(@props2[4] . " matches - " . @props2[7] . "\n");
      }
      if(($size1 == @props2[4]) && ($date1 == @props2[5]) && ($time1 == @props2[6]) && ($filename1 ne @props2[7]))
      {
         my($thisfilename) =  @props2[7];
         `rm /var/mail/virtual/james/cur/$thisfilename`;
          print("delete " . @props2[7] . "\n");
          @filelist2 = `$str`;
