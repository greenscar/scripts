...
sub formfu_edit :Chained('object') :PathPart('formfu_edit') :Args(0) :FormConfig('books/formfu_create.yml') 
{
   my ($self, $c) = @_;
...
sub delete :Chained('object') :PathPart('delete') :Args(0) {
   ...
   #$c->stash->{status_msg} = $c->stash->{object}->title . " deleted.";
   $c->flash->{status_msg} = $c->stash->{object}->title . " deleted.";
...
sub list : Local {
   ...
   my ($self, $c) = @_;
   $c->log->debug("Books::Local ($self, $c)");
   ...
   $c->log->debug("END Books::Local ($self, $c)");   
}
...
sub list_recent :Chained('base') :PathPart('list_recent') :Args(1) {
   ...
   $c->stash->{books} = [$c->model('DB::Book')->created_after(DateTime->now->subtract(days => $mins))];
...
sub list_recent_sub :Chained('base') :PathPart('list_recent_sub') :Args(2) {
   my ($self, $c, $mins, $substr) = @_;
   ...
   $c->stash->{books} = [$c->model('DB::Book')
                           ->created_after(DateTime->now->subtract(days => $mins))
                           ->title_like($substr)
                        ];
