...
moveOldXMLToArchive($XML_TEMP_DIR, $DEST_XML_BUILD_PROPS);

# Write properties file to new XML file
writeToFile($xml, $XML_TEMP_DIR, $DEST_XML_BUILD_PROPS, $dateTime);

# Compare the XML to its archive.
$compare_results_build_props= compareOldToNewXML($XML_TEMP_DIR, $DEST_XML_BUILD_PROPS);
